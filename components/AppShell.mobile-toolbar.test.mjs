...
test("uses a compact narrow-mobile toolbar with a floating action layer", () => {
  ...
  assert.doesNotMatch(source, /data-mobile-toolbar-action=(?:\{mobile \? )?"(?:system|tools|history|name|theme|language)"/);
});
...
test("keeps the mobile action layer open after using an expanded action", () => {
  ...
  assert.doesNotMatch(toggleTopPanel, /setMobileToolbarMoreOpen\(false\)/);
  assert.match(toggleTopPanel, /setMobileToolbarMoreOpen\(true\)/);
