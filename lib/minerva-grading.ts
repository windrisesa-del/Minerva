...
• Every saved item must include structured fields, not only prose. error_type is one of none, conceptual_error, procedural_error, careless_error, blank, or unreadable. knowledge_results is an array of {knowledge_id, result} using knowledge IDs from the question's knowledge_points or analysis.main_concepts; result is correct, incorrect, partial, or not_assessed. rubric_items records each scoring_criteria or partial_credit line as {requirement, score, max_score, hit}. feedback stays the human-readable basis and must not be the only place those facts exist.
