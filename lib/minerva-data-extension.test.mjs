...
const { classifyMinervaAttachment, coerceEvaluatorWrite, createMinervaDataExtension, shouldLoadAttachment } = await createJiti(import.meta.url).import("./minerva-data-extension.ts");

test("evaluator write coerces a top-level grading_result_id into source", () => {
  const payload = coerceEvaluatorWrite({
    buffer_items: [{
      candidate_id: "c1",
      evidence: [{
        evidence_id: "e1",
  ...
  assert.equal(payload.buffer_items[0].evidence[0].source.grading_result_id, "grade-1");

  const operationPayload = coerceEvaluatorWrite({
    operations: [{
      op: "set",
      path: "/evidence_buffer/candidate-2",
      value: {
        candidate_id: "candidate-2",
        evidence: [{ grading_result_id: "grade-2" }],
      },
    }],
  });
  assert.equal(operationPayload.operations[0].value.evidence[0].source.grading_result_id, "grade-2");
});
...
test("Evaluator tools enforce assignment, student, and submission scoped observation access", async (t) => {
  ...
  const readTool = tools.get("read_minerva");
  const writeTool = tools.get("write_minerva");
  const evaluatorWriteProperties = writeTool.parameters.properties;
  assert.ok(evaluatorWriteProperties.operations);
  assert.ok(evaluatorWriteProperties.evaluation_complete);
  assert.equal(evaluatorWriteProperties.profile_fields, undefined);
  assert.equal(evaluatorWriteProperties.buffer_items, undefined);
  assert.equal(evaluatorWriteProperties.change_notes, undefined);
  ...
  await writeTool.execute("write-3", {
    ...
    operations: [{
      op: "set",
      path: "/description/learning_trajectory/developing_abilities",
      value: ["开始在多步题中写出中间依据"],
      reason: "本次作业出现了完整中间步骤",
      evidence_refs: [{ grading_result_id: "grade-1" }],
    }],
  });
  ...
  assert.equal(body.operations[0].path, "/description/learning_trajectory/developing_abilities");
});
