...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(startupSource, /createMinervaDataExtension\(/);
  assert.match(startupSource, /graderAssignmentId: graderInfo\.assignmentId/);
  assert.match(startupSource, /graderStudentId: graderInfo\.studentId/);
  assert.match(startupSource, /graderSubmissionId: graderInfo\.submissionId/);
  assert.match(startupSource, /evaluatorAssignmentId: evaluatorInfo\.assignmentId/);
  assert.match(startupSource, /evaluatorStudentId: evaluatorInfo\.studentId/);
  assert.match(startupSource, /evaluatorSubmissionId: evaluatorInfo\.submissionId/);
  assert.match(startupSource, /preferPiWebSubagentExtension\(base\)/);
  assert.match(startupSource, /readGraderSessionData\(/);
  assert.match(startupSource, /GRADER_SYSTEM_PROMPT/);
  assert.match(startupSource, /MINERVA_GRADER_TOOLS/);
  assert.match(startupSource, /inner\.setActiveToolsByName\(\[\.\.\.MINERVA_GRADER_TOOLS\]\)/);
  assert.match(startupSource, /if \(summarizerInfo\) \{\s*inner\.setActiveToolsByName\(\["write_minerva"\]\)/);
  assert.match(startupSource, /EVALUATOR_SYSTEM_PROMPT/);
  assert.doesNotMatch(startupSource, /startEvaluatorAfterGrading\(/);
});
...
test("RPC session startup persists explicit preferences without replaying setters", async () => {
  ...
  assert.match(startupSource, /persistPreferences !== false/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /readSessionToolSelection\(sessionEntries\)/);
  assert.match(startupSource, /subagentResources\?\.tools \?\? persistedToolNames \?\? requestedToolNames/);
